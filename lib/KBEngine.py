"""
used to wait for player's operation,can only be used in coroutine methods(yield XXX)
args:
    cbNameOrDict: callbackName str or dict {callbackName1:overtime1,callbackName2:overtime2}
    overTime: overTime in seconds,if 0 wait forever,if cbNameOrDict is dict,this arg is useless,default 0
yield result:
    if cbNameOrDict is dict
    {
        callbackName1:WaitResult,
        callbackName2:WaitResult,
    }
    if cbNameOrDict is str
        WaitResult

example:
    Usage 1:
    result=yield WaitForCB("callback",10)
    #result's type is WaitResult
    #If received "callback" in 10 seconds,result.called>0 and result.args=(arg1,arg2)
    #If not received "callback" in 10 seconds, result.called==0 and result.args==None


    Usage 2:
    # wait for two callbacks,overtimes 10 and 20
    result=yield WaitForCB({"callback1":10,"callback2":20})
    #result is {"callback1":WaitResult1,"callback2":WaitResult2}

    #wait for player's operation callback,overtime is 10s
    result=yield WaitForCB({"callback3":10})
    #result is {"callback3":WaitResult}

"""
class WaitForCB:
    def __init__(self,cbNameOrDict,overTime=0):
        pass

"""
name:callback name
overTime
called:called times,if overtime,called==0.Normally it should be 1,if it's 2 or more,there maybe an error
args:tuple of returned args,if overtime,args==None,if there is only 1 arg,it's also a tuple:(arg,)
"""
class WaitResult:
    name=""
    overTime=0
...
"""
return successfullyCalled,returnValue
"""
def getCoroutineResult(generate):
    pass
...
class Entity:
    ...
    def setInterval(self,delay,repeat,callback):
        pass

    def delInterval(self,tid):
        pass
    ...
    def databaseInterfaceName( self ):
        ...
        """
        说明：
        databaseInterfaceName是实体持久化所在的数据库接口名称，该接口名称在kbengine_defs->dbmgr中配置。实体必须持久化过（databaseID>0）该属性才可用，否则返回空字符串。
...
class Proxy:
    ...
    def streamBytesToClient( self, data, desc="", id=-1 ):
        """
        功能说明：
...
def createEntityFromDBID( entityType, dbID, callback=None, dbInterfaceName="" ):
    """
    功能说明：
    从数据库里加载数据创建一个Base实体。
...
def createEntityAnywhereFromDBID( entityType, dbID, callback, dbInterfaceName=None ):
    """
    功能说明：
    从数据库里加载数据创建一个Base实体。
    服务端可能选择任何的Baseapp来创建Base实体。
    ...
    @callback
    这是一个可选的回调函数，当操作完成的时候它会被调用。回调函数带有3个参数：baseRef，databaseID和wasActive。
    如果操作成功，bzaseRef会是一个mailbox或者是新创建的Base实体的直接引用，databaseID会是实体的数据库ID，无论该实体是否已经激活
    wasActive都会有所指示，如果wasActive是True则baseRef是已经存在的实体的引用(已经从数据库检出)。如果操作失败这三个参数的值，baseRef将会是None，databaseID将会是0，wasActive将会是False。
    失败最常见的原因是实体在数据库里不存在，但偶尔也会出现其它错误比如说超时或者是分配ID失败。
...
def executeRawDatabaseCommand( command, callback, threadID=0, dbInterfaceName=0 ):
    """
    功能说明：
...
def listPathRes( path, extension=None ):
    """
    功能说明：
    获得一个资源目录下的资源列表。
    注意：资源必须在KBE_RES_PATH之下才可以访问到。
...
#需要测试
"""
callback 一个返回值,成功True,删除失败但是有entity返回entityCall或本机实体,无entity返回False
"""
def deleteEntityByDBID(entityType,dbid,pycallback,dbinterface=None):
    pass


"""
callback 一个返回值,entity存在的话返回entityCall或本机entity,entity不存在但是数据库存在True,都不存在False
"""
def lookUpEntityByDBID(entityType,dbid,pycallback,dbinterface=None):
    pass
